...
//Função global
__global__ void smoothing(PPMPixel *image, float n) {
...
int main(int argc, char *argv[]) {
    ...
    cudaMalloc(&d_i, size * sizeof(PPMPixel));

    //Copia a imagem para a GPU
    cudaMemcpy(d_i, image->data, size * sizeof(PPMPixel), cudaMemcpyHostToDevice);
    ...
    dim3 blocks(1,1, 1);
  	...
  	//Arredondando threads pra cima
  	blocks.x=((size)/THREADS_PER_BLOCK) + 1;
  	smoothing<<<blocks, threads>>>(d_i, size);
    t_end = rtclock();

    //Copia da GPU para a RAM
    cudaMemcpy(image->data, d_i, size * sizeof(PPMPixel), cudaMemcpyDeviceToHost);

    //Testar o tempo
    // printf("%lf", t_start - t_end);
